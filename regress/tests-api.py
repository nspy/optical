...
### Slice Creation: Ali
    print "Slice creation: Ali"
    lame_email = "ali@foo.com"
    policy = 'exact'
    if not s.api.createSlice("ali","aliPass","tcp:localhost:12345",lame_email) :
        print "Got false from creating slice for ali"
        test_failed("slice creation")
    x = s.api.getConfig("slices!ali!drop_policy")
    if (len(x) < 1) or x[0] != policy: 
        print "Failed to get drop_policy for ali: wanted " + policy + " but got " + str(x)
        test_failed("slice creation")
    print "     passed"
...
### Change slice drop_policy: Ali
    policy = "rule"
    print "Ali changeSlice (policy)"
    x = s.api.changeSlice("ali","drop_policy", policy)
    if not x:
        print "Got %s ; was expecting true" % (str(x))
        test_failed("changeSlice ali drop_policy test")
    print "passed"
...
#################################### Start fvadmin Tests
    ...
    print "Check drop_policy change"
    x = s.api.getConfig("slices!ali!drop_policy")
    if (len(x) < 1) or x[0] != policy: 
        print "Failed to get drop_policy for ali: wanted " + policy + " but got " + str(x)
        test_failed("change drop_policy")
    print "     passed"
