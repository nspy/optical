	...
	public void dispatch(HttpServletRequest req, HttpServletResponse resp) {
		...
		try {
			...
			try {
				...
				if (e.getCause() instanceof NoSliceForFlowSpace){
					NoSliceForFlowSpace noSliceEx = (NoSliceForFlowSpace)e.getCause();
					JSONResponse jResp = new JSONResponse(new JSONResult(gson.toJson(noSliceEx.getAllIDs(),
							noSliceEx.getAllIDs().getClass())), id);
					jResp.setMessage(noSliceEx.getJSONMessage());
					FVLog.log(LogLevel.DEBUG, null, "---------invoke ret:" + jResp.toString());
					try {
						writeJSONObject(resp, jResp);
					} catch (IOException e1) {
						FVLog.log(LogLevel.WARN, null, e1.getMessage(), e1);
					}
				}else{
					FVLog.log(LogLevel.WARN, null, e.getMessage(), e);
					throw new RPCException(RPCException.INTERNAL_ERROR, e.getMessage(), e);
