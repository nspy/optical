...
public class FVUserAPIImpl extends BasicJSONRPCService implements FVUserAPI {
	 ...
	 *
	 * Slices that contain the field separator are rewritten with underscoresl
	 *
	 * @param sliceName
	...
	@Override
	public Collection<Map<String, String>> getCLinks() {
		FVLog.log(LogLevel.DEBUG, null,
				"API getCLinks() by: " + APIUserCred.getUserName());
		Collection<Map<String, String>> clist = new LinkedList<Map<String, String>>();
		List<LinkAdvertisement> links = new ArrayList<LinkAdvertisement>();
		...
		for (FVEventHandler handler : FlowVisor.getInstance().getHandlersCopy()) {
			if (handler instanceof FVClassifier) {
			    System.out.print("FVUserAPIImpl.getCLinks:: FVClassifiers No: " + ++k);
			    OFFeaturesReply featuresReply = ((FVClassifier) handler).getSwitchInfo();
			    if ((featuresReply != null) && (featuresReply.getCPorts() !=null)) {
			    	System.out.println(" THIS IS A CIRCUIT SWITCH. Cports Number: " 
			    			+ featuresReply.getN_cports());
			    	fvClassifier = (FVClassifier) handler;
			    	for (LinkAdvertisement link : fvClassifier.getCircuitPortsBw()){
			    		if (link.getDstPort() != 0) {
			    			links.add(link);
			    			clist.add(link.toMap());
			    		}
			    	}								    
			    }
			    else 
			    	System.out.println(" packet switch");
			}
		}		
		return clist;
	}
		...
		for (FVEventHandler handler : fv.getHandlersCopy()) {
			if (handler instanceof FVClassifier) {
				...
				if (featuresReply != null) {
					//TODO maybe i have to improve this check - is it always valid 
					if (featuresReply.getCPorts() != null)
						continue;
					else {
...
//								"duplicate dpid detected: " + dpidStr);
	...
	@Override
	public List<String> listCDevices() {
		FVLog.log(LogLevel.DEBUG, null,
				"API listDevices() by: " + APIUserCred.getUserName());
		FlowVisor fv = FlowVisor.getInstance();
		// get list from main flowvisor instance
...
//		map.put("datapath", Long.toString(config.getDatapathId()));
		if (config != null) {
			if (config.getCPorts() != null){

				map.put("nCPorts", String.valueOf(config.getCPorts().size()));
				String portList = "";
				String portNames = "";
				String portNum = "";
				String wave = "";
				int p;
				for (Iterator<OFPhysicalCPort> it = config.getCPorts().iterator(); it
						.hasNext();) {
					OFPhysicalCPort cport = it.next();
					p = U16.f(cport.getPortNumber());
					portList += p;
					portNames += cport.getName() + "(" + p + ")";
//					datapath += cport.getPeerDatapathId();
//					peerPort += cport.getPeerPortNum();
					if (it.hasNext()) {
						portList += ",";
						portNames += ",";
						
					}
//					for (int j=0; j<config.getCPorts().get(i).getBandwidth().size(); ++j){
//						wave += ",";
//						portNum += ",";
//					}
				}
				for (int i=0; i<config.getCPorts().size(); ++i){
					for (int j=0; j<config.getCPorts().get(i).getBandwidth().size(); ++j){
//						map.put("portNum", Short.toString(config.getCPorts().get(i).getPortNumber()));
//						map.put("wave", Long.toString(config.getCPorts().get(i).getBandwidth().get(j)));
						portNum +=  config.getCPorts().get(i).getPortNumber();
						wave += config.getCPorts().get(i).getBandwidth().get(j);
					}
				...
				map.put("wave", wave);
//				map.put("peer dpid", datapath);
//				map.put("peerPort", peerPort);
//				for (int i=0; i<config.getCPorts().size(); ++i){
////					map.put("peer dpid",Long.toString((config.getCPorts().get(i).getPeerDatapathId())));
////					map.put("peer port no",Long.toString((config.getCPorts().get(i).getPeerPortNum())));
//					for (int j=0; j<config.getCPorts().get(i).getBandwidth().size(); ++j){
//						System.out.println("port num " + config.getCPorts().get(i).getPortNumber());
//						System.out.println("wave " + config.getCPorts().get(i).getBandwidth().get(j));
//						System.out.println("peer port " + config.getCPorts().get(i).getPeerPortNum());
						
//						map.put("port num", Short.toString(config.getCPorts().get(i).getPortNumber()));
//						map.put("wave", Long.toString(config.getCPorts().get(i).getBandwidth().get(j)));
//					}
...
//				}
	...
	public Map<String, String> getSliceInfo(String sliceName)
		...
		synchronized (FVConfig.class) {
			try {
				...
				map.put("drop_policy", FVConfig.getString(base + "drop_policy"));
			} catch (ConfigError e) {
	...
	public Boolean registerTopologyChangeCallback(String URL, String methodName, String cookie)
		...
		if (tc != null) {
			tc.registerCallBack(APIUserCred.getUserName(), URL, methodName, cookie, TopologyCallback.EventType.GENERAL.name());
			FVConfig.addTopologyCallback(URL, methodName, cookie, TopologyCallback.EventType.GENERAL, APIUserCred.getUserName());
			FlowVisor.getInstance().checkPointConfig();
			return true;
		} else
	...
	public Boolean unregisterTopologyChangeCallback() {
		TopologyController tc = TopologyController.getRunningInstance();
		if (tc != null) {
			String methodName = tc.getGeneralCallbackMethoName(APIUserCred.getUserName());
			tc.unregisterCallBack(APIUserCred.getUserName());
			FVConfig.removeTopologyCallback(methodName, TopologyCallback.EventType.GENERAL.name());
			FlowVisor.getInstance().checkPointConfig();
			return true;
		} else
			return false; // topology server not running
	...
	protected boolean confirmSlicesInFlowspaceExist(FlowEntry entry){
		boolean missingSlices = false;

		for(OFAction action : entry.getActionsList()){
			if(action instanceof SliceAction){
				String sliceName = ((SliceAction)action).getSliceName();
				if(!FVConfig.confirm(FVConfig.SLICES + FVConfig.FS + sliceName)){
					// Slice doesn't exist, log it
					FVLog.log(LogLevel.WARN, null, "Flowspace: " + entry.getId() + " references non-existent slice: " + sliceName);
					missingSlices = true;
				}
			}
		}
