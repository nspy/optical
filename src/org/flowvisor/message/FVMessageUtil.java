...
public class FVMessageUtil {
	...
	/**
	 * Is this slice allowed to use this list of actions with this ofconnect
	 * structure?
	 *
	 * Return a (potentially edited) list of actions or throw an exception if
	 * not allowed
	 ...
	 */
//	static public List<OFAction> approveActions(List<OFAction> actionList,
//			OFConnect connect, FVClassifier fvClassifier, FVSlicer fvSlicer)
//			throws ActionDisallowedException {
//		List<OFAction> approvedList = new ArrayList<OFAction>();
//
//		if (actionList == null)
//			return null;
//		for (OFAction action : actionList)
//			((SlicableAction) action).slice(approvedList, connect, fvClassifier,
//					fvSlicer);
//		return approvedList;
//	}
	...
	public static OFMessage makeErrorMsg(OFCFlowModFailedCode code, OFMessage msg) {
		OFError err = (OFError) FlowVisor.getInstance().getFactory()
				.getMessage(OFType.ERROR);
		err.setErrorType(OFErrorType.OFPET_CFLOW_MOD_FAILED);
		err.setErrorCode(code);
		err.setOffendingMsg(msg);
		return err;
	}
