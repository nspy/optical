...
public class LLDPUtil {
	...
	static public boolean handleLLDPFromSwitch(FVPacketIn pi,
		...
		for (FVSlicer fvSlicer : fvClassifier.getSlicers()) {
			configStr = FVConfig.SLICES + FVConfig.FS + fvSlicer.getSliceName()
					+ FVConfig.FS + FVConfig.SLICE_RECV_LLDP;
			try {
				if (fvSlicer.portInSlice(inport) && fvSlicer.isConnected() && FVConfig.getBoolean(configStr))
					fvSlicer.sendMsg(pi, fvClassifier);
			} catch (ConfigError ce) {
				FVLog.log(LogLevel.ALERT, fvClassifier, "Failed to retrieve lldp opt-in value from config."
						+ "\nSending LLDP packet to slice " +  fvSlicer.getSliceName() + " anyway");
				fvSlicer.sendMsg(pi, fvClassifier);
			}
		}
		return true;
