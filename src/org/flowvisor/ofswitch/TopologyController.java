...
public class TopologyController extends OFSwitchAcceptor {
	...
	private TopologyController(FVEventLoop pollLoop, int port, int backlog)
		...
		this.eventCallbacks.put(TopologyCallback.EventType.DEVICE_DISCONNECTED, new ArrayList<TopologyCallback>());
		this.eventCallbacks.put(TopologyCallback.EventType.PORT_ADDED, new ArrayList<TopologyCallback>());
		this.eventCallbacks.put(TopologyCallback.EventType.PORT_REMOVED, new ArrayList<TopologyCallback>());
	 ...
	 */
	public synchronized boolean registerCallBack(String user, String URL,String methodName,
		...
		if(eventType == TopologyCallback.EventType.GENERAL)
			...
			for(TopologyCallback callback : this.eventCallbacks.get(eventType)){
				if(callback.getMethodName().equals(methodName)){
					FVLog.log(LogLevel.WARN, null, "Topology callback not registered, already have a callback of this type with same method");
					return false;
				}
			}
	...
	public synchronized void deregisterCallback(String method, String callbackType){
		...
		List<TopologyCallback> callbacks = new ArrayList<TopologyCallback>(this.eventCallbacks.get(TopologyCallback.EventType.valueOf(callbackType)));
		for(TopologyCallback callback : callbacks){
	...
	public String getGeneralCallbackMethoName(String user){
		return this.generalCallBackDB.get(user).getMethodName();
	}
	...
	public void topoConnectionJustConnected(String dpidHex, String sockStr){
		topoConnectionDeviceChange(dpidHex, sockStr, TopologyCallback.EventType.DEVICE_CONNECTED);
	}

	public void topoConnectionBroken(String dpidHex){
		if(dpidHex.indexOf("topoDpid=") == -1){
			 // This was not a switch (possibly a controller? dont do a callback
			return;
		}
		// Start at index 9 to chop off Topodpid:
		topoConnectionDeviceChange(dpidHex.substring(9), "", TopologyCallback.EventType.DEVICE_DISCONNECTED);
	}

	private void topoConnectionDeviceChange(String dpidHex, String deviceIP, TopologyCallback.EventType type){
		JSONParam param = new JSONParam("dpid", dpidHex);
		ArrayList<JSONParam> params = new ArrayList<JSONParam>();
		params.add(param);
		param = new JSONParam("remote", deviceIP);
		params.add(param);
		for(TopologyCallback callback : eventCallbacks.get(type)){
			callback.setParams(params);
			callback.spawn();
		}
	}

	public void topoConnectionPortAdded(String dpidHex, int port, String portName){
		portChangeCallback(dpidHex, port, portName, TopologyCallback.EventType.PORT_ADDED);
	}

	public void topoConnectionPortRemoved(String dpidHex, int port, String portName){
		portChangeCallback(dpidHex, port, portName, TopologyCallback.EventType.PORT_REMOVED);
	}

	private void portChangeCallback(String dpidHex, int port, String portName, TopologyCallback.EventType type){
		JSONParam param = new JSONParam("dpid", dpidHex.substring(9));
		ArrayList<JSONParam> params = new ArrayList<JSONParam>();
		params.add(param);
		param = new JSONParam("port", port);
		params.add(param);
		param = new JSONParam("port_name", portName);
		params.add(param);
		for(TopologyCallback callback : eventCallbacks.get(type)){
			callback.setParams(params);
			callback.spawn();
	...
	public void disconnect(TopologyConnection topologyConnection) {
		...
		this.doCallback = true;
		this.topoConnectionBroken(topologyConnection.getName());
	}
