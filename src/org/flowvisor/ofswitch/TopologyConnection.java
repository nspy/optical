...
public class TopologyConnection implements FVEventHandler, FVSendMsg {
	...
	private final Map<Short, OFPhysicalCPort> phyCMap;
	...
	public TopologyConnection(TopologyController topologyController,
		...
		this.phyCMap = new HashMap<Short, OFPhysicalCPort>();
	}
	...
	public void init() throws IOException {
		...
		// OPT: ommit sending STATS_REQUEST message 
...
//		List<OFPhysicalCPort> cports = featuresReply.getCPorts();
...
//		if (cports != null && cports.size() < 1){
//			FVLog.log(LogLevel.WARN, this, "got circtuit switch with no cports!?!");
//			for (OFPhysicalCPort cport : cports)
//				this.addPort(cport);
//		}
		// schedule timer
		this.pollLoop.addTimer(new FVTimerEvent(System.currentTimeMillis()
		...
		topologyController.topoConnectionJustConnected(HexString.toHexString(this.featuresReply.getDatapathId()), sock.socket().getRemoteSocketAddress().toString());
	}
	...
	synchronized public void addPort(OFPhysicalPort port) {
		...
		this.slowIterator = this.slowPorts.iterator();
		topologyController.topoConnectionPortAdded(this.name, port.getPortNumber(), port.getName());
	}
	
	synchronized public void addPort(OFPhysicalCPort cport) {
		// this function is synchronized so it shouldn't get hosed
		FVLog.log(LogLevel.DEBUG, this, "sending init probe to cport "
				+ cport.getPortNumber());
		
//		FIXME:logic!!
//		sendLLDP(port);
//		this.slowPorts.add(Short.valueOf(port.getPortNumber()));
		this.phyCMap.put(Short.valueOf(cport.getPortNumber()), cport);
//		this.slowIterator = this.slowPorts.iterator();
		topologyController.topoConnectionPortAdded(this.name, cport.getPortNumber(), cport.getName());
	}
	...
	synchronized public void removePort(OFPhysicalPort port) {
		...
		if (this.slowPorts.contains(port)) {
			...
			this.slowIterator = this.slowPorts.iterator();
			topologyController.topoConnectionPortRemoved(this.name, port.getPortNumber(), port.getName());
...
//	FIXME:correct logic
	synchronized public void removePort(OFPhysicalCPort cport) {
		// this function is synchronized so it shouldn't get hosed
		if (this.slowPorts.contains(cport)) {
			this.slowPorts.remove(cport);
			this.slowIterator = this.slowPorts.iterator();
			topologyController.topoConnectionPortRemoved(this.name, cport.getPortNumber(), cport.getName());
		} else if (this.fastPorts.contains(cport)) {
			this.fastPorts.remove(cport);
			// no iterator to update
			topologyController.topoConnectionPortRemoved(this.name, cport.getPortNumber(), cport.getName());
		} else
			FVLog.log(LogLevel.WARN, this,
